...
// Include the HTTP client (and its Mongoose + STL dependencies) before gsc.hpp:
// gsc.hpp -> declarations.hpp #defines `level` as a macro, which would otherwise
// corrupt identifiers named `level` inside the STL headers.
